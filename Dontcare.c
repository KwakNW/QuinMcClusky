...
int main(void)
{
	...
	for (k = 0; k < Minterm; k++)
	{
		for (s = 1; s < Minterm - k; s++)
		{
			...
			if (sum == 1)
			{
				...
				a++;
				//}
				arr_name[p][0] = totalterm[k];
				arr_name[p][1] = totalterm[k + s];
	...
	for (int k = 0; k < Minterm - dontcare; k++)
	{
		printf("%#5d", term[k]);
	}
	...
	for (int h = 0; h < Acount; h++) {
		for (int k = 0; k < Minterm - dontcare; k++) {
			if (Xterm[k] == 1) {
				if (newarr_name[h][0] == term[k] || newarr_name[h][1] == term[k]) {
					for (int kk = 0; kk < Minterm - dontcare; kk++) {
						if (Tterm[h][kk] == 1) {
	...
	for (int h = 0; h < Tcount; h++) {
		...
		for (int j = 0; j < Minterm - dontcare; j++) {
			fsum += Tterm[h][j];
		}
